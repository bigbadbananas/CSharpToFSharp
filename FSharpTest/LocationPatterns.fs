...
module LocationPatterns = 
    ...
    let routePattern = String.Format("^(?<{0}>", ROUTE_GROUP_KEY) + @"((OR|I|US)-*)?\d{1,3}\w?)$"
    let highwayPattern = String.Format("^(?<{0}>", HIGHWAY_GROUP_KEY) + @"(H\d{1,3}))$"
    ...
    let roadDirCodePattern = 
        String.Format("(?<{0}>(.*))", PREDECESSOR_GROUP_KEY) + String.Format("(?<{0}>", ROAD_DIRECTION_CODE_KEY) 
        + @"(\b(NB|EB|WB|SB)\b))"

    let lanesAffectedSingle = @"OF|ON|RA|All|[A-FIMSX]"
    let lanesAffectedPattern = 
        String.Format("(?<{0}>(.*))", PREDECESSOR_GROUP_KEY) + String.Format("(?<{0}>", LANES_AFFECTED_KEY) 
        + @"(?<=\s)(OF|ON|RA|All|[A-FIMSX:])+(?=\s|$))"

    let districtIdsPattern = 
        String.Format("(?<{0}>(.*))", PREDECESSOR_GROUP_KEY) + String.Format("(?<{0}>", DISTRICT_IDS_KEY) 
        + @"(?<=\s)(-[\w\d]{1,2}(,\s?[\w\d]{1,2})*$))"
    let regionIdPattern = String.Format("(?<{0}>", REGION_ID_KEY) + @"(?<=R)\d"
    let latLngPattern = String.Format("^(?<{0}>", LATLONG_GROUP_KEY) + @"(=-?\d+(.\d+)?\s*,\s*-?\d+(.\d+)?))$"
